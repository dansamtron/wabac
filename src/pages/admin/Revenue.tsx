  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-[#6b6b6b]">Transaction fee {data.fee.percentage}% + fixed ₦{data.fee.fixed} per paid order. Every order shows seller earnings vs platform fee.</p>
      </div>
        ...
        <div className="mt-3 space-y-2">
          {data.breakdown.slice(0, 15).map((b) => (
            <div key={b.orderId} className="flex flex-wrap gap-3 justify-between rounded-xl border border-[#F3E6D3] p-3">
              <div>
                <div className="text-sm font-bold font-mono">#{b.orderId.slice(-6).toUpperCase()} • Seller {b.sellerId.slice(-6)}</div>
                <div className="text-xs text-[#6b6b6b]">{b.customerName} • {new Date(b.createdAt).toLocaleDateString()}</div>
              </div>
              <div className="text-right text-sm">
                <div>Total ₦{b.total.toLocaleString()} • Fee ₦{b.fee.toLocaleString()} • Seller ₦{b.sellerEarning.toLocaleString()}</div>
              </div>
            </div>
          ))}
