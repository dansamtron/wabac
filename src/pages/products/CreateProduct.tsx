...
export default function CreateProduct() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasVariants) {
      if (variants.length === 0) {
        setError("Add at least one variant (size/color)")
        return
      }
      for (let i = 0; i < variants.length; i++) {
        ...
        if (!Number.isInteger(st) || st < 0) {
          setError(`Variant ${i + 1}: stock must be 0 or more`)
          return
        }
      }
    }
    ...
    if (discountActive) {
      ...
      if (discountType === "percentage" && (dv < 1 || dv > 90)) {
        setError("Percentage discount must be 1 - 90")
        return
      }
    }
    setLoading(true)
    try {
      const discount: ProductDiscount | undefined = discountActive ? { active: true, type: discountType, value: Math.round(Number(discountValue)) } : undefined
  ...
  const previewPrice = (() => {
    ...
    if (discountType === "percentage") return Math.max(1, Math.round(base * (1 - dv / 100)))
    return Math.max(1, base - dv)
  })()
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="rounded-2xl bg-white border border-[#F3E6D3] p-6 space-y-7">
        ...
        <div className="rounded-2xl border border-[#F3E6D3] p-4 space-y-3">
          ...
          {!hasVariants ? (
            <p className="text-xs text-[#6b6b6b]">Enable if this product needs size, color or SKU. Example fashion, shoes.</p>
          ) : (
            <>
              <p className="text-xs text-[#6b6b6b]">Add combinations. At least one of size, color or SKU per row. Stock is per variant.</p>
                ...
                {variants.map((v, idx) => (
                  <div key={v.id} className="grid grid-cols-12 gap-2 items-end rounded-xl bg-[#FFFBF5] border border-[#F3E6D3] p-3">
                    ...
                    <label className="col-span-4 sm:col-span-2">
                      ...
                      <input type="number" min={0} value={v.stock} onChange={(e) => updateVariant(idx, { stock: Number(e.target.value) || 0 })} className="mt-1 w-full rounded-lg border border-[#F3E6D3] px-2 py-2 text-sm outline-none" />
                    </label>
                    <label className="col-span-4 sm:col-span-2">
                      <span className="text-[11px] font-bold">Price override</span>
                      <input type="number" min={0} value={v.price || ""} onChange={(e) => updateVariant(idx, { price: e.target.value ? Number(e.target.value) : undefined })} placeholder="blank = base" className="mt-1 w-full rounded-lg border border-[#F3E6D3] px-2 py-2 text-sm outline-none" />
                    </label>
