...
export default function Revenue() {
  ...
  const sellerEarnings = totalSales - platformFee
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#6b6b6b]">Views sales and revenue information — orders, payouts and top products. Platform fee is mock 5 percent for Phase 8 preview.</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="rounded-2xl bg-white border border-[#F3E6D3] p-5">
          ...
          <div className="mt-2 text-2xl font-bold">₦{sellerEarnings.toLocaleString()}</div>
          <div className="text-xs text-[#6b6b6b]">After 5% platform fee</div>
        </div>
        <div className="rounded-2xl bg-white border border-[#F3E6D3] p-5">
          ...
          <div className="text-xs text-[#6b6b6b]">5% mock for preview</div>
        </div>
        <div className="rounded-2xl bg-white border border-[#F3E6D3] p-5">
          ...
          <div className="text-xs text-[#6b6b6b]">{deliveredOrders} delivered</div>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-[1.6fr_0.9fr] gap-6">
        <div className="rounded-2xl bg-white border border-[#F3E6D3] p-5">
          ...
          <div className="mt-4 rounded-xl bg-[#FFFBF5] border border-[#F3E6D3] p-3 text-xs leading-5 text-[#6b6b6b]">
            Chart uses paid order totals scoped by sellerId. Phase 8 will add configurable transaction fee and platform transaction records.
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl bg-white border border-[#F3E6D3] p-5">
            ...
            <div className="mt-4 space-y-3">
              {topProducts.length === 0 ? (
                <div className="text-xs text-[#6b6b6b]">No sales yet. Create a test order.</div>
              ) : (
                topProducts.map((p) => (
          ...
          <div className="rounded-2xl bg-[#1a1a1a] text-white p-6">
            ...
            <p className="text-xs leading-5 text-white/70 mt-1">Payouts are triggered after Paystack confirmation. For now all Paid orders are considered settled to your bank shown in Settings.</p>
            <div className="mt-4 rounded-xl bg-white/10 p-3 text-sm">
              <div className="flex justify-between"><span className="text-white/70">Next payout</span><span className="font-bold">₦{sellerEarnings.toLocaleString()}</span></div>
          ...
          {paidOrders.slice(0, 5).map((o) => (
            <Link key={o.id} to={`/dashboard/orders/${o.id}`} className="flex items-center justify-between rounded-xl border border-[#F3E6D3] p-3 hover:bg-[#FFFBF5]">
              <div>
                ...
                <div className="text-xs text-[#6b6b6b]">{new Date(o.createdAt).toLocaleDateString()} • {o.items.length} items</div>
              </div>
